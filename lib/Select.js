...
var Select = (_temp = _class = function (_React$Component) {
  ...
  Select.getDerivedStateFromProps = function getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (valueRefresh) {
      ...
      if (!newState.value) {
        ...
        var _formatInternalValue = (0, _util.formatInternalValue)(newState.value),
            selectorValueList = _formatInternalValue.selectorValueList,
            selectorValueMap = _formatInternalValue.selectorValueMap; // 考虑多选
  ...
  */
  ...
  this.triggerChange = function (status, id, item, selectedArray) {
    var onSelectorChange = _this2.props.onSelectorChange;
  ...
  this.onMenuMultipleSelect = function (record, status) {
    ...
    if (record) {
      ...
      _this2.triggerChange(!status, record[valueField], record, _this2.state.selectorValueList);
    }
  };
